        ...
        for row in rows:
            ...
            if allow:
                ...
                if not cfgchange and port in ServerPool.get_instance().tcp_ipv6_servers_pool:
                    ...
                    for name in merge_config_keys:
                        if (name in cfg) and (
                                (name not in relay._config) or not self.cmp(cfg[name], relay._config[name])):
                            cfgchange = True
                            break
...
class Dbv3Transfer(DbTransfer):
    ...
    def update_all_user(self, dt_transfer):
                ...
                try:
                    if id in self.port_uid_table:
                        cur.execute(
                            ...
                            str(self.port_uid_table[id]) + "', '" + str(transfer[0]) + "', '" + str(
                                transfer[1]) + "', '" + \
                            str(self.cfg["node_id"]) + "', '" + str(self.cfg["transfer_mul"]) + "', '" + \
                            self.traffic_format(
                                (transfer[0] + transfer[1]) * self.cfg["transfer_mul"]) + "', unix_timestamp()); ")
                except:
        ...
        if self.update_node_state:
            try:
                ...
                try:
                    cur.execute(
                        "INSERT INTO `" + self.ss_node_info_name + "` (`id`, `node_id`, `uptime`, `load`, `log_time`) VALUES (NULL, '" + \
    ...
    def pull_db_users(self, conn):
        ...
        if self.update_node_state:
            ...
            try:
                cur.execute("SELECT " + ','.join(node_info_keys) + " FROM ss_node where `id`='" + str(
                    self.cfg["node_id"]) + "'")
