...
class tls_ticket_auth(plain.plain):
    ...
    def client_encode(self, buf):
        ...
        if self.handshake_status == 0:
            ...
            data = self.tls_version + self.pack_auth_data(
                self.server_info.data.client_id) + b"\x20" + self.server_info.data.client_id + binascii.unhexlify(
            ...
            ext += b"\x00\x23" + struct.pack('>H', len(self.server_info.data.ticket_buf[host])) + \
    ...
    def client_decode(self, buf):
        ...
        verify = buf[11:33]
        if hmac.new(self.server_info.key + self.server_info.data.client_id, verify, hashlib.sha1).digest()[:10] != buf[
                                                                                                                   33:43]:
            raise Exception('client_decode data error')
        if hmac.new(self.server_info.key + self.server_info.data.client_id, buf[:-10], hashlib.sha1).digest()[
           :10] != buf[-10:]:
            raise Exception('client_decode data error')
        return (b'', True)
    ...
    def server_encode(self, buf):
        ...
        if (self.handshake_status & 8) == 8:
            ...
            return ret
        self.handshake_status |= 8
        data = self.tls_version + self.pack_auth_data(self.client_id) + b"\x20" + self.client_id + binascii.unhexlify(
            b"c02f000005ff01000100")
    ...
    def server_decode(self, buf):
        ...
        if (self.handshake_status & 1) == 1:
            ...
            if hmac.new(self.server_info.key + self.client_id, verify[:verify_len], hashlib.sha1).digest()[
        ...
        if self.max_time_dif > 0 and (time_dif < -self.max_time_dif or time_dif > self.max_time_dif \
                                      or common.int32(
                    utc_time - self.server_info.data.startup_time) < -self.max_time_dif / 2):
